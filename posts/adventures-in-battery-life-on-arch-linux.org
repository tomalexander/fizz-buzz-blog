...
So far, all of my changes have been based on imperceptible changes to my experience using the laptop. For funsies, I decided I should see what power usage I am at when I let powerdown reduce my backlight (which is automatically does on battery power, but I've been turning the backlight back to max).
#+BEGIN_SRC sh
